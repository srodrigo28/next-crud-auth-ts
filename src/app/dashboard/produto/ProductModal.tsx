// src/app/dashboard/produto/ProductModal.tsx
"use client"
...
interface ProductModalProps {
  ...
  onProductSaved: (product: ProductData) => void
}
...
export default function ProductModal({
  ...
  useEffect(() => {
    if (isOpen) {
      if (productData) {
        ...
        setFormData({ nome: "", descricao: "", preco: 0, imagem: null })
        setFormattedPrice("")
  ...
  const handlePriceChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const formattedNumber = isNaN(number) ? 0 : number

    setFormattedPrice(formatToBRL(formattedNumber))
    setFormData((prev) => ({ ...prev, preco: formattedNumber }))
  }
  ...
  const handleSaveProduct = async (e: FormEvent) => {
    ...
    try {
      ...
      let imageUrl = productData?.imagem ?? null
      ...
      const productDataToSave: Partial<ProductData> = {
        nome: formData.nome,
      ...
      onProductSaved(saveResult.data as ProductData)
      onClose()
    } catch (err: unknown) {
      console.error("Erro ao salvar produto:", err)
      const errorMessage = err instanceof Error ? err.message : "Erro ao salvar produto. Tente novamente."
